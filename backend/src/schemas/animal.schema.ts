...
const compareDateOnly = (left: string, right: string): boolean => {
	return left <= right;
};

const AnimalBaseSchema = zod
	.object({
		name: zod.string().trim().min(1, 'Name is required').max(150, 'Name too long (max 150)').openapi({
			description: 'Animal name',
			example: 'Max',
		}),

		breed: zod.string().trim().min(1, 'Breed is required').max(255, 'Breed too long (max 255)').openapi({
			description: 'Animal breed',
			example: 'Labrador',
		}),

		birth_date: DateOnlySchema.openapi({
			description: 'Birth date',
			example: '2020-05-10',
		}),

		adoption_date: DateOnlySchema.openapi({
		...
		color: zod.string().trim().max(100, 'Color too long (max 100)').optional().openapi({
...
export const CreateAnimalPayloadSchema = AnimalBaseSchema.omit({ user_id: true }).refine((data) => compareDateOnly(data.birth_date, data.adoption_date), {
	...
	.refine(
		(data) => {
			if (data.birth_date && data.adoption_date) {
				return compareDateOnly(data.birth_date, data.adoption_date);
			}
			return true;
		},
		{
