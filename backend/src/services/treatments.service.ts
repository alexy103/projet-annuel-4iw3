...
function assertAnimalAccess(
  animal: Animal,
  callerId: number,
  role: string,
): void {
  if (role === "admin") return;
  if (role === "user" && animal.user_id !== callerId)
    throw new AppError("Access denied", 403);
  if (role === "clinic" && !animal.is_shared)
    throw new AppError("Access denied", 403);
}
...
export const treatmentService = {
  ...
  async update(
    ...
    const existingTreatment: Treatment =
      await treatmentsRepository.findById(treatmentId);
    if (!existingTreatment) throw new AppError("Treatment not found", 404);
    if (role !== "admin") {
      const animal = await animalsRepository.findById(
        existingTreatment.animal_id,
      );
      assertAnimalAccess(animal, callerId, role);
    }
  ...
  async delete(
    ...
    const remindersCount = (
      await treatmentRemindersRepository.findByTreatmentId(treatmentId)
    ).length;
    if (remindersCount > 0)
      throw new AppError("Cannot delete treatment: reminders associated", 409);
